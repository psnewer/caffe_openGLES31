...
void PoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  glUniform1i(glGetUniformLocation(progHandle, "in_width"), bottom[0]->width());
  glUniform1i(glGetUniformLocation(progHandle, "in_height"), bottom[0]->height());
  glUniform1i(glGetUniformLocation(progHandle, "kernel_width"), kernel_w_);
  glUniform1i(glGetUniformLocation(progHandle, "kernel_height"), kernel_h_);
  glUniform1i(glGetUniformLocation(progHandle, "padding"), pad_w_);
  glUniform1i(glGetUniformLocation(progHandle, "stride"), stride_w_);
...
template <typename Dtype>
void PoolingLayer<Dtype>::forward_pool(const GLuint input, const GLuint output) {
       glBindBufferBase(GL_SHADER_STORAGE_BUFFER,1,input);
       glBindBufferBase(GL_SHADER_STORAGE_BUFFER,2,output);
       glUseProgram(progHandle);
       glDispatchCompute(pooled_width_, pooled_height_, channels_);
}
